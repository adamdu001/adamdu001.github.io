...
def parse_events(events_dir):
    """Parse event items from the _events directory."""
    events = []
    
    if not os.path.exists(events_dir):
        return events
    
    for event_file in sorted(glob.glob(os.path.join(events_dir, "*.md"))):
        ...
        if front_matter_match:
            ...
            event_entry = {
                "name": front_matter.get('title', ''),
                "category": front_matter.get('collection', 'events'),
                "date": front_matter.get('event_date', front_matter.get('date', '')),
...
def create_cv_json(md_file, config_file, repo_root, output_file):
    ...
    # Add events
    cv_json["events"] = parse_events(os.path.join(repo_root, "_events"))
